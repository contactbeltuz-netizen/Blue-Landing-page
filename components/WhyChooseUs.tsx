      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row items-end justify-between mb-16 gap-6">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 bg-[#ff6c00]/10 px-4 py-2 rounded-full text-[#ff6c00] font-black text-xs mb-6 uppercase tracking-[0.3em]">
              <CheckCircle2 className="w-4 h-4" />
              Our Excellence
            </div>
            <h2 className="text-4xl md:text-6xl font-black text-[#1a2b47] leading-tight tracking-tight">
              Why Travelers <br /> Choose Elegant?
            </h2>
          </div>
          <p className="text-slate-500 text-lg font-medium max-w-sm border-l-4 border-[#ff6c00] pl-6 py-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {REASONS.map((reason, index) => (
            <div 
              key={index} 
              className="group relative bg-slate-50 p-10 rounded-[3rem] transition-all duration-500 hover:bg-[#1a2b47] hover:shadow-2xl hover:-translate-y-2 flex flex-col items-start"
            >
              <div className="mb-8 p-4 bg-white rounded-2xl text-[#ff6c00] group-hover:bg-[#ff6c00] group-hover:text-white transition-all duration-500 shadow-sm">
                {reason.icon}
              </div>
              <h3 className="text-2xl font-black text-[#1a2b47] group-hover:text-white mb-4 transition-colors">
                {reason.title}
              </h3>
              <p className="text-slate-500 group-hover:text-slate-300 font-medium leading-relaxed transition-colors">
                {reason.description}
              </p>
